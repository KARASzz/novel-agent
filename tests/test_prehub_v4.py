...
def test_prehub_no_rag_does_not_call_tavily(monkeypatch, tmp_path):
    ...
    kb_dir.mkdir()
    (kb_dir / "local.md").write_text("都市复仇 逆袭 打脸 前三集钩子 付费点", encoding="utf-8")
...
def test_prehub_missing_tavily_env_has_explicit_fallback(monkeypatch, tmp_path):
    ...
    kb_dir = tmp_path / "knowledge_base"
    kb_dir.mkdir()
    (kb_dir / "method.md").write_text("短剧 逆袭 情绪债 反转 付费点", encoding="utf-8")
...
def test_context_bundle_serializes_for_parser(tmp_path):
    ...
    assert payload["prompt_injection_payload"]["system_addition"].startswith("[PreHub V4 ContextBundle]")
