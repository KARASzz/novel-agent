# 《红星锚定》九步章节生产线完整母版 v2.2

> 用途：新对话开章、章节生产、压测与正式正文施工。  
> 版本：v2.2  
> 核心变化：从第1步开始编号；将“本体论文字树形图”和“ToT多路径发散”合并为第3步。  
> 使用方式：每章一个新对话。开章第一条直接粘贴本母版，并填入 `<current_chapter>` 与 `<previous_chapter_script>`。  
> 规则：本母版为完整母版，不提供简化版。正式生产全程使用本版。
...
第1步是自动备料。  
它从当前章节名、上一章第9步回写、当前项目包、增量补丁、生产线、美学基座、旧版资产库中抽取本章需要的变量。  
它不写正文，不写输入卡以外的施工内容，不扩写百科。
...
第2步把第1步抽出来的变量压成当前章节的施工指令。  
它不是设定仓库。  
它只回答：本章要完成什么，不能发生什么，上一章带来什么，下一章必须承接什么，本章爽点是什么，最大风险是什么。
...
第3步合并两个动作：  
先用文字树形图把本章对象拆开，再基于树形图生成ToT候选路径。  
这一步的核心不是炫推理，而是把本章锁在人物、规则、信息、流程、后果几个硬节点上。
...
第5步把X/Y双线转成六个可执行节拍。  
每节拍约对应正文500字。  
每个节拍都必须有触发事件、人物动作、信息变化、流程卡点、可见后果和失败控制。  
没有这些，就是气氛节拍，要砍或重写。
...
每次只写两个节拍，约1000字。  
写完后按单要素迭代，不顺手全改。  
单要素顺序固定：事件推进、身体感受、环境变化、旁人反应、流程卡点、去AI腔与句子口感。
...
第7步不是普通润色。  
它负责从读者座位检查文本：冷启动、断点、钩子密度、爽点外化、情绪曲线、复杂设定降噪。  
它必须指出问题位置，并给出修订版正文。
...
它是下一章用的连载导航条。  
控制在150到250字。  
它必须记录本章走向、新挖坑、填掉坑、本章爽点、爽点状态，以及必要的伏笔、回声、规则状态、人物状态、降级记录。
